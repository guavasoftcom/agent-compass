...
const LogEntry = ({ log, spanStartMs }: { log: LogRow; spanStartMs: number }) => {
  ...
  // assistant_response's `response` body is markdown prose, not a JSON/plain
  // value — it always gets a "view formatted" link (regardless of length) that
  // opens in the markdown-rendered modal rather than the JSON one.
  const isAssistantResponse = eventName === 'assistant_response';
      ...
      {expanded ? (
        <Box sx={{ pb: 1, pl: 1, display: 'flex', flexDirection: 'column', gap: 0.4 }} onClick={(e) => e.stopPropagation()}>
          ...
          {detailEntries.map(([k, v]) => {
            const isResponseBody = isAssistantResponse && k === 'response';
            return (
              <Box key={k} sx={{ display: 'grid', gridTemplateColumns: 'minmax(140px,auto) 1fr', gap: 1.5, typography: 'mono', fontSize: 11.5 }}>
                ...
                <LongAttrValue
                  ...
                  force={isResponseBody}
                  format={isResponseBody ? 'markdown' : 'json'}
                />
              </Box>
            );
          })}
