  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75 }}>
        {events.map((ev, i) => (
          <Box key={i} sx={{ border: 1, borderColor: 'divider', borderRadius: radii.sm, p: 1.1, bgcolor: 'background.paper' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <Box component="span" sx={{ typography: 'mono', fontSize: 10.5, color: 'text.disabled' }}>
                T+{formatDuration((Date.parse(ev.timestamp) - spanStartMs) * 1e6)}
              </Box>
              <Box component="span" sx={{ typography: 'mono', fontSize: 12, fontWeight: 600 }}>{ev.name}</Box>
            </Box>
          </Box>
        ))}
      </Box>
