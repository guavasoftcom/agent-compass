...
interface Props {
  ...
  error?: string | null;
  expanded: Set<number>;
  /** IDs of rows that were just prepended by the live tail — rendered with a flash animation. */
  freshRowIds: Set<number>;
  onToggleExpand: (id: number) => void;
  onLoadMore: () => void;
}

const LogStreamView = ({ rows, total, loading, hasMore, error, expanded, freshRowIds, onToggleExpand, onLoadMore }: Props) => {
  const theme = useTheme();
  ...
  // Defensive: rows should always be an array, but never let a bad value crash the page.
  const safeRows = rows ?? [];
  const empty = !safeRows.length && !loading && !error;
  ...
  return (
    <>
      <Box ref={scrollRef} onScroll={handleScroll} sx={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
        ...
        {error && !safeRows.length ? (
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 1.5, color: 'text.secondary', textAlign: 'center', p: 5 }}>
            <Typography sx={{ fontFamily: "'Sora', sans-serif", color: 'error.main', fontSize: 16, fontWeight: 700 }}>
              Couldn’t load logs
            </Typography>
            <Typography sx={{ fontSize: 13.5 }}>{error}</Typography>
            <Typography sx={{ fontSize: 13.5, color: 'text.disabled' }}>Try narrowing the time window or reloading.</Typography>
          </Box>
        ) : null}
        ...
        {safeRows.map((row) => {
          ...
          const isFresh = freshRowIds.has(row.id);
            ...
            <Box key={row.id} sx={{ contentVisibility: 'auto', containIntrinsicSize: 'auto 38px' }}>
              <Box
                ...
                sx={{
                  ...
                  ...(isFresh
                    ? {
                        animation: 'logRowFlash 1.8s ease-out forwards',
                        '@keyframes logRowFlash': {
                          '0%': { backgroundColor: alpha(theme.palette.success.light, 0.28) },
