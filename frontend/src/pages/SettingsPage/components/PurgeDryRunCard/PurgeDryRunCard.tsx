...
const RETENTION_OPTIONS = [30, 60, 90, 180].map((days) => ({
  value: days,
  label: `${days}d`,
}));
...
const PurgeDryRunCard = ({
  ...
  // Resets the disclosure when the retention window changes, without the extra
  // render an effect-based reset would cost — the documented alternative to
  // "setState in an effect" for state that depends on a changed prop.
  const [retentionDaysAtLastRender, setRetentionDaysAtLastRender] =
    useState(retentionDays);
  if (retentionDays !== retentionDaysAtLastRender) {
    setRetentionDaysAtLastRender(retentionDays);
    setShowSql(false);
  }
    ...
    <Paper variant="outlined" sx={{ p: '22px 24px' }}>
      ...
      >
        <Typography
          ...
          sx={{ lineHeight: 1.5 }}
        >
          Nothing prunes old telemetry on its own. Pick a window to see what
          removing everything older would reclaim, then either run the SQL
          yourself or purge from here — the button deletes permanently, behind
          a confirmation.
        </Typography>
        <SegmentedToggle
      ...
      {!purgePreview && isPurgePreviewLoading ? (
        ...
        <>
              ...
              <TableBody>
                {purgePreview.tables.map((table) => (
                  <TableRow key={table.tableName} hover>
                    ...
                    <TableCell
                      sx={numberCell}
                    >{`${table.sharePercent.toFixed(1)}%`}</TableCell>
                    <TableCell sx={numberCell}>
          ...
          >
            ...
            >
              <li>
                A session is deleted only once it has no activity anywhere —
                logs, tokens, or traces — for the whole window above. A session
                with any recent activity keeps every row, including its oldest
                ones: sessions are never split between old and recent.
              </li>
              <li>
                A{' '}
                ...
                makes the space reusable by Postgres but does not return it to
                the operating system. Only{' '}
                <Box component="code" sx={{ typography: 'mono' }}>
                  VACUUM FULL
                </Box>{' '}
                shrinks the files, and it takes an exclusive lock plus room for
                a full rewrite.
              </li>
              <li>
                Inside a session that does get purged, the newest row of every
                metric counter is kept regardless of age, as a safeguard: if
                that session ever emitted again, its next value wouldn&apos;t be
                booked as one giant spike.
              </li>
              <li>
                ...
                setting (default 30 days, controlling how long a session stays
                resumable locally) — purge more aggressively than that and a
                resumed session&apos;s history can already be gone from this
                dashboard even though the conversation itself still resumes
                fine.
              </li>
            </Box>
          </Box>
          ...
          >
            <GhostButton onClick={() => setShowSql(!showSql)}>
              {showSql ? 'Hide SQL' : 'View SQL'}
            </GhostButton>
            <GhostButton onClick={handleCopy}>
              {hasCopied ? 'Copied' : 'Copy SQL'}
            </GhostButton>
            <GhostButton
              tone="danger"
          ...
          {purgeResult && (
            ...
            >
              ...
              <Typography variant="body2" sx={{ lineHeight: 1.6 }}>
                Deleted{' '}
                <Box component="b">
                  {formatCompact(purgeResult.totalRowsDeleted)}
                </Box>{' '}
                rows from sessions dormant for {purgeResult.retentionDays}+
                days, in{' '}
                {(purgeResult.durationMillis / MILLISECONDS_PER_SECOND).toFixed(
                  1,
                )}
                s
                {purgeResult.preservedRows > 0 && (
                  <>
                    , keeping{' '}
                    <Box component="b">
                      {formatCompact(purgeResult.preservedRows)}
                    </Box>{' '}
                    rows — sessions still active elsewhere, plus a marker per
                    counter so live streams stay correct
                  </>
                )}
                . Database size{' '}
                {formatBytes(purgeResult.databaseTotalBytesBefore)} →{' '}
                {formatBytes(purgeResult.databaseTotalBytesAfter)} — the space
                is now reusable by Postgres but still allocated on disk. To hand
                it back, run{' '}
                <Box component="code" sx={{ typography: 'mono' }}>
...
const PurgeDryRunSkeleton = () => (
  <>
        ...
        <TableBody>
          {Array.from({ length: SKELETON_TABLE_ROW_COUNT }, (_, rowIndex) => (
            <TableRow key={rowIndex}>
              ...
              <TableCell sx={numberCell}>
                <Skeleton variant="text" width={64} sx={{ ml: 'auto' }} />
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
    ...
    <Skeleton
      variant="rounded"
      sx={{ mt: 2, height: 108, borderRadius: 1.5 }}
    />
    ...
    <Skeleton
      variant="rounded"
      sx={{ mt: 1.5, height: 120, borderRadius: 1.5 }}
    />
  </>
);
