...
(function () {
  ...
  function renderTokCard(tokAgg, tokTotal, modelCalls, traceCost) {
    const el = $('tokcard');
    if (tokTotal <= 0) { el.innerHTML = '<span class="toknone">No model tokens — this trace made no model calls.</span>'; return; }
  ...
  function renderMinimap() {
    const mt = $('mmtrack');
    const ticks = SPANS.map((s) => {
      const left = (s.off / TOTAL) * 100, w = Math.max(0.6, (s.dur / TOTAL) * 100);
      const top = 2 + depth.get(s.spanId) * 10;
      const col = s.status === 'error' ? 'var(--bad)' : SVC_COLOR[s.scope] || 'var(--primary)';
      return '<div class="mmtick" style="left:' + left + '%;width:' + w + '%;top:' + top + 'px;background:' + col + '"></div>';
    }).join('');
    const bl = (viewStart / TOTAL) * 100, bw = (vspan() / TOTAL) * 100;
    mt.innerHTML = ticks + '<div class="brush" id="brush" style="left:' + bl + '%;width:' + bw + '%"><span class="bh l"></span><span class="bh r"></span></div>';
    wireBrush();
  }
  function wireBrush() {
    ...
    const downAt = (e, mode) => {
      ...
      drag = { mode, x: e.clientX, vs: viewStart, ve: viewEnd, w: mt.getBoundingClientRect().width };
    ...
    const move = (e) => {
      ...
      else { ve = Math.max(drag.ve + dt, vs + MIN); ve = Math.min(TOTAL, ve); }
      viewStart = vs; viewEnd = ve;
      renderAxis(); renderWaterfall(); updateBrush();
    };
    ...
    brush.addEventListener('mousedown', (e) => downAt(e, 'move'));
    brush.querySelector('.bh.l').addEventListener('mousedown', (e) => downAt(e, 'l'));
    brush.querySelector('.bh.r').addEventListener('mousedown', (e) => downAt(e, 'r'));
  }
  function updateBrush() {
    const brush = $('brush');
    if (brush) { brush.style.left = ((viewStart / TOTAL) * 100) + '%'; brush.style.width = ((vspan() / TOTAL) * 100) + '%'; }
  }
  $('minimap').addEventListener('dblclick', () => { viewStart = 0; viewEnd = TOTAL; renderAxis(); renderWaterfall(); updateBrush(); });
  ...
  function renderWaterfall() {
    ...
    visibleFiltered().forEach((s) => {
      ...
      const chips =
        (fullRateTok > 0 ? '<span class="chip tok" data-tip><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3" fill="currentColor"/></svg>' + fmtTok(fullRateTok) + '</span>' : '') +
        ...
        (tok.cacheRead > 0 ? '<span class="chip cr" data-tip><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 12a9 9 0 0 1 15-6.7L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-15 6.7L3 16"/><path d="M3 21v-5h5"/></svg>' + fmtTok(tok.cacheRead) + '</span>' : '') +
        (spanCost > 0 ? '<span class="chip cost" title="' + (isRollupCost ? 'Cost of the requests made under this span' : 'Cost of this model call') + '">' + fmtUsd(spanCost) + '</span>' : '') +
        (modelName ? '<span class="chip mdl" title="' + esc(effort ? modelName + ' · ' + effort + ' effort' : modelName) + '">' + esc(shortModelName(modelName)) + (effort ? '<em>· ' + esc(effort) + '</em>' : '') + '</span>' : '') +
        (s.attrs && s.attrs.tool_name ? '<span class="chip tool" data-tip>' + esc(s.attrs.tool_name) + '</span>' : '') +
        (s.status === 'error' ? '<span class="chip err">error</span>' : '') +
        (below > 0 ? '<span class="chip below">+' + below + ' below</span>' : '');
