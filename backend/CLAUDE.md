...
- `ingest/` — OTLP protobuf mappers (`OtlpLogMapper`, `OtlpMetricMapper`, `OtlpTraceMapper`) for `POST /v1/logs`, `/v1/metrics`, `/v1/traces`. Mappers are `@Component`s, not MapStruct — they translate OTLP proto into `*Entity` rows including the attribute jsonb maps.
...
- `service/` — `LogIngestService` / `MetricIngestService` / `TraceIngestService` for write paths; `LogQueryService` / `MetricQueryService` / `TraceQueryService` are `@Transactional(readOnly = true)` and own the parsing/cursor/range logic for dashboard endpoints; `ReportService` renders the markdown report from a Mustache template.
- `controller/` — `OtlpLogsController` / `OtlpMetricsController` / `OtlpTracesController` accept `application/x-protobuf` and return an OTLP success/partial-success response. `DashboardController` is the single REST surface for the React app under `/api`; `ReportController` serves the markdown report.
- `model/` — record DTOs (Lombok `@Data @Builder` on older shapes like `ToolCallCount` / `TraceSummary`); MapStruct (`spring` component model, `lombok-mapstruct-binding` keeps them compatible) handles entity → DTO mapping for `LogRecord` / `Span` / `EventRow`.
...
- `/postgresql-optimization` — when writing or tuning native SQL in `repository/`, especially anything touching the `jsonb` attribute columns: containment (`@>`), GIN/`jsonb_path_ops` indexes, window functions (`date_bin`, `percentile_cont`, `LAG`), and range filters. The aggregations in [LogRecordRepository.java](src/main/java/com/guavasoft/telemetry/repository/LogRecordRepository.java) are the canonical examples.
- `/java-refactoring-extract-method` — when a Java method exceeds the skill's thresholds (LOC > 15, NOM > 10, CC > 10). Most relevant in `service/` (multi-step aggregation pipelines, `ReportService.buildSuggestions`) and `ingest/` (OTLP proto → entity mapping).
